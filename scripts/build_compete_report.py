...
def run_flyai(cmd):
    ...
    try:
        ...
        if result.returncode == 0 and result.stdout.strip():
            ...
            log_err(f"flyai 执行失败: {result.stderr[:200] if result.stderr else '无输出'}")
            return None
    except subprocess.TimeoutExpired:
        log_err("flyai 执行超时（60秒）")
        return None
    except json.JSONDecodeError as e:
        log_err(f"flyai JSON解析失败: {e}")
        return None
    except Exception as e:
        log_err(f"flyai 执行异常: {e}")
        return None
...
def fetch_fliggy_by_keywords(poi_data, check_in, check_out):
    """
    通过飞猪关键词搜索获取竞品价格
    ...
    for poi_name, poi_info in poi_data.items():
        ...
        holiday_raw = run_flyai(cmd_holiday)
...
def fetch_fliggy_price(hotel_name, check_in, check_out, scope, target_count, poi_data=None):
    ...
    holiday_data = run_flyai(holiday_cmd)
...
def main():
    ...
    if poi_data:
        ...
        keyword_competitors = fetch_fliggy_by_keywords(poi_data, check_in, check_out)
